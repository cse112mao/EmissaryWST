      ...
      function insertTestDefinition(mtoken, element, position) {
        ...
        if (maskdef && !escaped) {
          ...
          for (var prevalidators = maskdef.prevalidator, prevalidatorsL = prevalidators ? prevalidators.length : 0, i = 1; i < maskdef.cardinality; i++) {
            var prevalidator = prevalidatorsL >= i ? prevalidators[i - 1] : [], validator = prevalidator.validator,
  ...
  function maskScope(actionObj, maskset, opts) {
        ...
        function handleMatch(match, loopNdx, quantifierRecurse) {
          ...
          if (void 0 !== match.matches) {
            if (match.isGroup && quantifierRecurse !== match) {
              ...
              var maltMatches, alternateToken = match, malternateMatches = [], currentMatches = matches.slice(),
                loopNdxCnt = loopNdx.length, altIndex = ndxInitializer.length > 0 ? ndxInitializer.shift() : -1;
    ...
    function determineLastRequiredPosition(returnDefinition) {
      var pos, testPos, buffer = getBuffer(), bl = buffer.length, lvp = getLastValidPosition(), positions = {},
        lvTest = getMaskSet().validPositions[lvp], ndxIntlzr = void 0 !== lvTest ? lvTest.locator.slice() : void 0;
            ...
            function validateRegexToken(token, fromGroup) {
              ...
              for (var mndx = 0; mndx < token.matches.length; mndx++) {
                ...
                if (matchToken.isGroup === !0) isvalid = validateRegexToken(matchToken, !0); else if (matchToken.isQuantifier === !0) {
                  var crrntndx = $.inArray(matchToken, token.matches), matchGroup = token.matches[crrntndx - 1],
